(function() {
        ...
        for(var i = 0; i < char_array.length; i++) {
            ...
            for(var j = 0; j < current_node.children.length; j++) {
                ...
                if(child.char == _char) {
                    ...
                    if(child.code && (/\s/.test(next_char))) {
                        result.text += ' ' + self.printEmoId(child.code, 'html');
                        result.emoticons.push(child.code);
                        // its a match
                        found = true;
                        matched_chars = [];
                    }
            ...
            if(!matched) {
                ...
                if(matched_chars.length) {
                    ...
                    result.text += matched_chars.join('') + _char;
                    matched_chars = [];
                } else {
                    result.text += _char;
                }
    ...
    p.parseText = function(text) {
        var result = this.processText(text);
        return result.text;
    };

    p.listEmoticonsInText = function(text) {
        var result = this.processText(text);
